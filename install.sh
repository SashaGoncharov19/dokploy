...
# This is the Bun fork of Dokploy: https://github.com/SashaGoncharov19/dokploy-bun
# Images are published to this repository's GitHub Container Registry, and version
# detection follows this repository's releases - not upstream's. Override either
...
detect_version() {
    ...
    if [ -z "$version" ]; then
        ...
        version=$(curl -fsSL --connect-timeout 10 -o /dev/null -w '%{url_effective}\n' \
            https://github.com/${DOKPLOY_REPO}/releases/latest 2>/dev/null | \
            sed 's#.*/tag/##')
        ...
        # like <image>:https://... Accept only values that look like a
...
install_dokploy() {
    ...
    DOCKER_IMAGE="${DOKPLOY_IMAGE}:${VERSION_TAG}"
